...
struct CombatInputSyntheticTests {
    ...
    static func request(_ log: SyntheticLog, tribes: (any LobbyTribesProvider)? = nil, cards: CardDB? = nil) throws
        ...
        var engine = TavernEngine(cards: cards, simulatorTribes: tribes)
    ...
    @Test("With a pool, the simulator gets the tribe inference's lobby (here settled by the hero-pick banner)")
    func resolvedTribesFromPool() throws {
        let banner: [HS.Race] = [.aberration, .demon, .elemental, .murloc, .quilboar]
        func run(reading: ScreenTribeReading?) throws -> CombatSimulationRequest {
            var engine = TavernEngine(pool: PoolFixture.pool, timeZone: TimeZone(identifier: "UTC")!)
            var read = false
            for line in Self.combat().lines {
                engine.ingest(line)
                if !read, engine.state.game != nil, let reading {
        ...
        let resolved = try run(reading: ScreenTribeReading(tribes: banner))
        #expect(resolved.input.gameState.validTribes == banner.map(\.rawValue).sorted())
        #expect(resolved.tribesKnown)
        // A pool but no evidence yet: not a whole lobby, so every tribe (not the seen-minion stand-in).
        let unknown = try run(reading: nil)
        #expect(unknown.input.gameState.validTribes == nil && !unknown.tribesKnown)
    }
