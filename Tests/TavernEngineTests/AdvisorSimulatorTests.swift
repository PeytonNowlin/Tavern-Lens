...
@Suite("Advisor on the simulator", .timeLimit(.simulationWait))
struct AdvisorSimulatorTests {
    ...
    func seeded() async throws {
        ...
        let budget = SimulationBudget(simulations: 100, maxDurationMilliseconds: 600_000, intermediateResults: 50)
    ...
    func golden() async throws {
        ...
        // Partway, as a bookmark would have caught it: replaying that many evaluations gives the same
        // advice. (A few evaluations are enough; `AdvisorSyntheticTests.replayPrefix` tries more cut points.)
        let partway = try #require(reports.first { $0.evaluations == 4 })
    ...
    func combatFirst() async throws {
        ...
        try await waitUntil { (runner.current?.evaluations ?? 0) >= 2 }
        ...
        let odds = try await simulator.simulate(
            combat, budget: .init(simulations: 200, maxDurationMilliseconds: 600_000, intermediateResults: 50)
        )
        #expect(odds.isFinal && odds.simulations == 200)
        // The advisor asks for nothing more once cancelled, and its evaluation in flight stops at its
        // next step: the combat waits for at most that, never for the rest of the plan.
        #expect(runner.current == nil)
        #expect(log.finishedCount <= scored + 1, "at most the evaluation in flight finished after the cancel")
